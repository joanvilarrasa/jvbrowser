        ...
        if hasattr(child_node, 'children'):
            for child in child_node.children:
                if isinstance(child, str):
                    child_acc = AccessibilityNode(child, self)
                else:
                    if child.tag == "iframe" and hasattr(child, 'frame') and child.frame and child.frame.loaded:
                        child_acc = FrameAccessibilityNode(child, self)
                    else:
                        child_acc = AccessibilityNode(child, self)
                self.children.append(child_acc)
...
class FrameAccessibilityNode(AccessibilityNode):
    def __init__(self, node, parent=None):
        ...
        if hasattr(node, 'layout_object') and node.layout_object:
            self.scroll = node.frame.scroll
            self.zoom = node.layout_object.zoom
        else:
            self.scroll = 0
            self.zoom = 1.0
    
    def hit_test(self, x, y):
        bounds = self.bounds[0] if self.bounds else None
        if not bounds or not bounds.contains(x, y): 
            return None
